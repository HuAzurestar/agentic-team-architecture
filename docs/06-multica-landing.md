...
Autopilot 是**可复用的自动化规则**（不是 Agent 本身）：trigger 触发 → 派发 → 由对应 Agent / Squad 执行。

- **节奏性 Autopilot**：迭代节奏（规划/评审/复盘）、站会进度同步、例行检查——按 schedule 触发。
- **推动型 Autopilot**：AS 的高频简单推动可由 Autopilot 承载（或由 Assistant AS 执行），阻塞时上报。
- **规则管理**：创建、更新、查看、触发均通过 `multica autopilot` 命令；**不用于测试性触发**（trigger 是真实副作用）。
...
- **上下文容器**：Project 承载资源（github_repo / local_directory）与项目描述，注入每次任务上下文。
